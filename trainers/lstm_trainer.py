...
import time
from utils import get_linear_burn_in_fn, get_exp_decay, RolloutBuffer, RecurrentRolloutBuffer
from variables import procgen_envs
from model import LSTMImpalaAgent
...
class LSTMPPO(OnPolicyAlgorithm):
    """
    ...
    def _setup_model(self) -> None:
        super(LSTMPPO, self)._setup_model()
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            generator = self.rollout_buffer.get(batch_size=self.config['batch_size'])

            for rollout_data in generator:

                valid_mask = rollout_data.valid_mask.bool()
                hiddens = rollout_data.hiddens.float()
                observations = rollout_data.observations.float()
                actions = rollout_data.actions
                advantages = torch.masked_select(rollout_data.advantages, valid_mask).float()
                old_values = torch.masked_select(rollout_data.old_values, valid_mask).float()
                old_log_prob = torch.masked_select(rollout_data.old_log_prob, valid_mask).float()
                returns = torch.masked_select(rollout_data.returns, valid_mask).float()

                if isinstance(self.action_space, spaces.Discrete):
                    # Convert discrete action from float to long
                    actions = actions.long()

                values, log_prob, entropy = self.policy.evaluate_actions(observations, actions, hiddens)
                values = torch.masked_select(values.squeeze(), valid_mask)
                log_prob = torch.masked_select(log_prob, valid_mask)
                entropy = torch.masked_select(entropy, valid_mask)
    ...
    def collect_rollouts(self, env: VecEnv, callback: BaseCallback, rollout_buffer: RolloutBuffer,
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                self.policy.hidden = self._last_hiddens
                self.policy.cell = self._last_cells
                action, value, log_prob, hidden = self.policy(observations)
                self._last_hiddens = self.policy.hidden
                self._last_cells = self.policy.cell
            ...
            self._last_hiddens[:, self._last_episode_starts, :] = 0.
            self._last_cells[:, self._last_episode_starts, :] = 0.
